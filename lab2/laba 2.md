...
1 Настроил политику маршрутизации для сетей офиса
2 Распределил трафик между двумя линками с провайдером.
(Весь трафик с 172.168.1.2 уходит на R2, но если ip sla покажет не доступность R2, трафик пойдет в R7. 
Весь трафик с 172.168.1.1 по PBR ухотит к R7 всегда.)
3 Настроил отслеживание линка через технологию IP SLA.(только для IPv4)
